...
static void ColouriseBalsaDoc(unsigned int startPos, int length, int initStyle, WordList *keywordlists[],
                            Accessor &styler) {
...
static void FoldNoBoxBalsaDoc(unsigned int startPos, int length, int initStyle,
                            Accessor &styler) {
	bool foldComment = styler.GetPropertyInt("fold.comment") != 0;
	bool foldPreprocessor = styler.GetPropertyInt("fold.preprocessor") != 0;
	bool foldCompact = styler.GetPropertyInt("fold.compact", 1) != 0;
	bool foldAtElse = styler.GetPropertyInt("fold.at.else", 0) != 0;
        // Balsa specific folding options:
        // fold_at_module -
        //      Generally used methodology in balsa code is
		...
		if (foldPreprocessor && (style == SCE_V_PREPROCESSOR)) {
			...
			if (ch == '{') {
                ...
                if (style == SCE_V_WORD && stylePrev != SCE_V_WORD) {
                        unsigned int j = i;
                        if (styler.Match(j, "procedure")) {
                                levelNext -= levelNext % 256;
                                levelNext += 256;
			}
			else if (styler.Match(j, "if") || 
                            styler.Match(j, "arbitrate") || 
                            styler.Match(j, "select") || 
                            styler.Match(j, "case") || 
                            styler.Match(j, "loop") || 
                            styler.Match(j, "begin")) {
                                levelNext++;
			}
			else if (styler.Match(j, "then")) {
			  int pos = j;
			  char c;
			  do {
			    pos --;
			  ...
			  if (styler.SafeGetCharAt(pos-1) == '-' && c == '>')
			    levelNext++;
                        }
			else if (styler.Match(j, "end") && !IsAWordChar(styler.SafeGetCharAt(j+3))) {
                                levelNext--;
				if (levelNext%256 == 0)
				  levelNext -= 256;
                        }
...
static void FoldBalsaDoc(unsigned int startPos, int length, int initStyle, WordList *[],
                       Accessor &styler) {
	FoldNoBoxBalsaDoc(startPos, length, initStyle, styler);
}

static const char * const balsaWordLists[] = {
...
LexerModule lmBalsa(SCLEX_AUTOMATIC, ColouriseBalsaDoc, "balsa", FoldBalsaDoc, balsaWordLists);
